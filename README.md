...
- メールはmailtrapを使用していますので送受信ご確認の際は下記にてログインください
  - https://mailtrap.io/
    - ログイン(メアド):pleiades_tm@yahoo.co.jp
      - (参考)アカウントID：2330889
    - パスワード　：Test1@laravel
  - なお、大変恐縮ですが、無料アカウントのsandbox制限で9月は残り5件しか受信できません。
    - テストコードなどではメール送信部分は含めておらず自動送信はされません。このためご確認される際は明示的に画面からの「送信」ボタンにて送信をお願いします。
      - 管理者等からのお知らせの場合、全ユーザーや来店履歴ありのユーザーで複数宛先の場合でも、メール送信の場合は１件のメールです。
      - 一方、リマインダーで送信する場合、ユーザーごとに1件ずつ送られます。このため予約当日am8:00にスケジュールしています。
        - 対象の予約・ユーザーが複数いたら、その人数分メールが送られます。
        - 予約はFactoryで作成しているためランダムです。必要に応じて画面から予約の追加・変更をお願いします。
...
- その他
  - テストコードについては、基本機能分までは作成しています。
  - 追加実装分はテストコードを作成せずに、手動でテスト実施しております。
    - 上述のmailtrapの制限もありますが、時間が足りなかったためです。ご容赦ください
    - なお、追加実装後に、基本機能部分のリグレッションテスト（テストコード改修も含む）は実施済です。
